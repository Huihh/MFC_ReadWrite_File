...
int rsa_pub_pkcs1_encrypt(unsigned char *n, unsigned char *e, unsigned char *in, unsigned char *out, unsigned long len)
{
	int ret = 0;
	rsa_context rsa;
	havege_state hs;
	...
	mpi_read_binary(&rsa.N, n, 256);
	//MPI_CHK(mpi_read_binary(&rsa.N, n, 256));
	mpi_read_binary(&rsa.E, e, 4);
	//MPI_CHK(mpi_read_binary(&rsa.E, e, 4));
	...
	ret = rsa_pkcs1_encrypt(&rsa, havege_random, &hs, RSA_PUBLIC_SIGN, len, in, out);
...
int rsa_pub_pkcs1_encrypt2(unsigned char *n, unsigned char *e, unsigned char *in, unsigned char *out, unsigned long len)
{
	int ret = 0;
	rsa_context rsa;
	havege_state hs;
	...
	ret = rsa_pkcs1_encrypt(&rsa, havege_random, &hs, RSA_PUBLIC, len, in, out);
