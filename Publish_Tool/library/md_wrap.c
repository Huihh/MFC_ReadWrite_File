...
static int sm3_file_wrap( const char *path, unsigned char *output )
{
     ((void) path);
     ((void) output);
     return( POLARSSL_ERR_MD_FEATURE_UNAVAILABLE );
...
static void sm3_hmac_starts_wrap( void *ctx, const unsigned char *key,
                                  size_t keylen )
{
     sm3_hmac_starts( (sm3_context *) ctx, key, keylen );
}

static void sm3_hmac_update_wrap( void *ctx, const unsigned char *input,
                                  size_t ilen )
{
     sm3_hmac_update( (sm3_context *) ctx, input, ilen );
}

static void sm3_hmac_finish_wrap( void *ctx, unsigned char *output )
{
     sm3_hmac_finish( (sm3_context *) ctx, output );
}

static void sm3_hmac_reset_wrap( void *ctx )
{
     sm3_hmac_reset( (sm3_context *) ctx );
}

static void sm3_hmac_wrap( const unsigned char *key, size_t keylen,
                           const unsigned char *input, size_t ilen,
                           unsigned char *output )
{
...
{
     sm3_context *ctx;
     ctx = (sm3_context *) polarssl_malloc( sizeof( sm3_context ) );
...
const md_info_t sm3_info = {
     ...
     sm3_wrap,
     sm3_file_wrap,
     sm3_hmac_starts_wrap,
     sm3_hmac_update_wrap,
     sm3_hmac_finish_wrap,
     sm3_hmac_reset_wrap,
     sm3_hmac_wrap,
     sm3_ctx_alloc,
     sm3_ctx_free,
