                                            ...
                                            size_t ilen)
{
    ...
    char dn[X509_MAX_DN_NAME_SIZE] = {0};
    ...
    while( signer_info != NULL )
    {
        ...
        if( 0 > x509_dn_gets( dn, X509_MAX_DN_NAME_SIZE, &crt->subject ) )
        {
            return( POLARSSL_ERR_PKCS7_INVALID_FORMAT );
        }
        if( strstr( dn, "title=" ) && POLARSSL_PK_RSA == pk_get_type( pk ) )
        {
        ...
        {
            ret = hash_msg_with_user_id( (sm2_context*)((pk_ec(*pk))),
                                         input, ilen,
