...
{
	unsigned char   key[16], iv[12], hdr[_MaxText_Length_], ptx[_MaxText_Length_], ctx[_MaxText_Length_],tag[16];
...
/*
//测试示例：
char Key[100]="",IV[50]="",HDR[100]="",PlainText[512]="",Cliper[512]="",Tag[50]="";
sprintf(Key,"000102030405060708090A0B0C0D0E0F");
sprintf(IV,"4D4D4D0000BC614E01234567");
sprintf(HDR,"10D0D1D2D3D4D5D6D7D8D9DADBDCDDDEDFC0010000080000010000FF0200");
sprintf(PlainText,"");
Encrypt_StringData(Key,IV,HDR,PlainText,Cliper,Tag);
TRACE("认证模式-加密:T:%s \r\n",Tag);
sprintf(Tag,"");
Decrypt_StringData(Key,IV,HDR,Cliper,Tag,PlainText);
TRACE("认证模式-解密:T:%s \r\n",Tag);

sprintf(HDR,"");
sprintf(PlainText,"C0010000080000010000FF0200");
AES_gcm.Test_Encrypt_StringData(Key,IV,HDR,PlainText,Cliper,Tag);
TRACE("加密模式-加密：Cliper:%s  \r\n",Cliper);
sprintf(PlainText,"");
Decrypt_StringData(Key,IV,HDR,Cliper,Tag,PlainText);
TRACE("加密模式-解密:Plain:%s \r\n",PlainText);


sprintf(HDR,"30D0D1D2D3D4D5D6D7D8D9DADBDCDDDEDF");  
sprintf(PlainText,"C0010000080000010000FF0200");
Encrypt_StringData(Key,IV,HDR,PlainText,Cliper,Tag);
TRACE("认证加密模式-加密：Cliper:%s    T:%s \r\n",Cliper,Tag);
sprintf(PlainText,"");
sprintf(Tag,"");
Decrypt_StringData(Key,IV,HDR,Cliper,Tag,PlainText);
TRACE("认证加密模式-解密:Plain:%s  T:%s \r\n",PlainText,Tag);

sprintf(Key,"000102030405060708090A0B0C0D0E0F");
sprintf(PlainText,"00112233445566778899AABBCCDDEEFF");
Cliper[0]=0;
aes_wrap_String(Key,PlainText,Cliper);
TRACE("aes_wrap-加密:Cliper:%s  \r\n",Cliper);

PlainText[0]=0;
aes_unwrap_string(Key,Cliper,PlainText);
TRACE("aes_unwrap-解密:Cliper:%s  \r\n",PlainText);

//输出结果
认证模式-加密:   T:06725d910f9221d263877516 
认证模式-解密:   T:06725d910f9221d263877516 
加密模式-加密：  Cliper:411312FF935A47566827C467BC  
加密模式-解密:   Plain:c0010000080000010000ff0200 
认证加密模式-加密：Cliper:411312FF935A47566827C467BC    T:7d825c3be4a77c3fcc056b6b 
认证加密模式-解密:  Plain:c0010000080000010000ff0200    T:7d825c3be4a77c3fcc056b6b 
aes_wrap-加密:     Cliper:1FA68B0A8112B447AEF34BD8FB5A7B829D3E862371D2CFE5  
aes_unwrap-解密:    Cliper:00112233445566778899AABBCCDDEEFF  
*/